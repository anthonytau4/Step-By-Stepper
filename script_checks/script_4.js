...
(function(){
  ...
  const EXTRA_NOTES = [
    ...
    'New: the Choreography section now doubles as Featured Dances, with Gmail-ready submission buttons for sending dances in properly.',
  ...
  function ensureHost(){
    ...
    host.innerHTML = `<div class="space-y-5"><section id="${SAVED_PAGE_ID}" hidden></section><section id="${WHATSNEW_PAGE_ID}" hidden></section></div>`;
  ...
  function saveCurrentDanceToCloud(){
    ...
    if (!profile) {
      alert('Sign in with Google on this page first.');
      return false;
    }
    const data = readAppData();
  ...
  function renderSavedDancesPage(){
    ...
    const googleCard = profile ? `
      <div class="rounded-2xl border p-5 sm:p-6 ${theme.panel}">
        <div class="flex flex-wrap items-start justify-between gap-4">
          <div class="min-w-0">
            <div class="flex items-center gap-3">
              <span class="stepper-extra-tab-icon">${iconGoogle()}</span>
              <div>
                <div class="text-lg font-black tracking-tight">Signed in with Google front end</div>
                <p class="mt-1 text-sm ${theme.subtle}">${escapeHtml(profile.name)} · ${escapeHtml(profile.email)}</p>
              </div>
            </div>
            <p class="mt-4 text-sm leading-relaxed ${theme.subtle}">This is the front-end shell for Google sign-in and cloud save. It already remembers this account on this device, and a live backend can be wired in later without ripping the page apart.</p>
          </div>
          <div class="flex flex-wrap gap-3">
            ...
            <button type="button" data-action="signout-google" class="stepper-mini-btn ${theme.button}">Sign out</button>
          </div>
        </div>
      </div>` : `
      <div class="rounded-2xl border p-5 sm:p-6 ${theme.panel}">
        <div class="flex items-center gap-3">
          ...
          <div>
            ...
            <p class="mt-1 text-sm ${theme.subtle}">Front-end only for now. Pick the Google name and email you want this device to remember.</p>
          </div>
        </div>
        <div class="mt-5 grid grid-cols-1 sm:grid-cols-2 gap-3">
          <label class="rounded-2xl border px-4 py-3 ${theme.soft}"><span class="text-[10px] font-black uppercase tracking-widest ${theme.subtle}">Google Name</span><input data-google-name type="text" placeholder="Anthony Tau" class="mt-2 w-full bg-transparent outline-none font-semibold" /></label>
          <label class="rounded-2xl border px-4 py-3 ${theme.soft}"><span class="text-[10px] font-black uppercase tracking-widest ${theme.subtle}">Google Email</span><input data-google-email type="email" placeholder="you@gmail.com" class="mt-2 w-full bg-transparent outline-none font-semibold" /></label>
        </div>
        <div class="mt-4 flex flex-wrap items-center gap-3">
          <button type="button" data-action="signin-google" class="stepper-mini-btn ${theme.orange}"><span class="stepper-extra-tab-icon">${iconGoogle()}</span><span>Continue with Google</span></button>
          <p class="text-sm ${theme.subtle}">This saves a front-end profile on this device so the cloud-save tab has a user to attach things to.</p>
        </div>
      </div>`;
    ...
    const localCards = localDances.length ? localDances.map(item => `
      <article class="rounded-3xl border p-5 sm:p-6 ${theme.soft}" data-local-dance-id="${escapeHtml(item.id)}">
        ...
        <div class="mt-4 flex flex-wrap items-center gap-3">
          ...
          ${profile ? `<button type="button" data-action="push-local-cloud" class="stepper-mini-btn ${theme.orange}">Push to cloud</button>` : ''}
    ...
    `).join('') : `<div class="rounded-3xl border p-6 sm:p-8 text-center ${theme.soft}"><p class="text-lg font-bold">No cloud saves yet.</p><p class="mt-2 ${theme.subtle}">${profile ? 'Use the cloud button above or push one of your saved dances up into the cloud front end.' : 'Sign in with Google above first so this device knows which cloud shelf to use.'}</p></div>`;
        ...
        <div class="rounded-2xl border p-5 ${theme.panel}">
          <div class="flex items-center gap-3">
            ...
            <div>
              ...
              <p class="mt-1 text-sm ${theme.subtle}">This shell is ready for a real Google/cloud backend later. Right now it still behaves usefully by remembering your saved dances on this device and grouping cloud saves under the Google profile you pick above.</p>
            </div>
          </div>
        </div>
    ...
    const signInBtn = page.querySelector('[data-action="signin-google"]');
    if (signInBtn) signInBtn.addEventListener('click', () => {
      const nameField = page.querySelector('[data-google-name]');
      const emailField = page.querySelector('[data-google-email]');
      const email = String(emailField && emailField.value || '').trim();
      const name = String(nameField && nameField.value || '').trim();
      if (!email) {
        alert('Add a Google email first.');
        return;
      }
      saveGoogleFrontendProfile({ name, email });
      renderSavedDancesPage();
    });

    const signOutBtn = page.querySelector('[data-action="signout-google"]');
    if (signOutBtn) signOutBtn.addEventListener('click', () => {
      clearGoogleFrontendProfile();
      renderSavedDancesPage();
    });
  ...
  function renderChoreographyPanel(targetId){
    ...
    const theme = themeClasses();
    const currentSubmitHref = buildFeaturedSubmissionLink(getCurrentDanceEntry());
    const cards = featured.length ? featured.map(item => {
      const submitHref = buildFeaturedSubmissionLink(item);
      return `
      <article class="rounded-3xl border p-5 sm:p-6 ${theme.soft}">
        ...
        <div class="mt-4 flex flex-wrap items-center justify-between gap-3">
          <p class="text-xs font-semibold uppercase tracking-widest ${theme.subtle}">Updated ${escapeHtml(formatDate(item.updatedAt))}</p>
          <a href="${escapeHtml(submitHref)}" class="inline-flex items-center justify-center rounded-full px-4 py-2 text-xs font-black uppercase tracking-widest ${theme.dark ? 'bg-orange-500 text-neutral-950' : 'bg-orange-500 text-white'}">Submit via Gmail</a>
        </div>
      </article>
    `}).join('') : `
      <div class="rounded-3xl border p-6 sm:p-8 text-center ${theme.soft}">
        ...
        <a href="${escapeHtml(currentSubmitHref)}" class="inline-flex items-center justify-center rounded-full px-5 py-3 mt-5 text-xs font-black uppercase tracking-widest ${theme.dark ? 'bg-orange-500 text-neutral-950' : 'bg-orange-500 text-white'}">Open Gmail submission</a>
      </div>
    `;
    page.className = `rounded-3xl border shadow-sm overflow-hidden ${theme.shell}`;
    page.innerHTML = `
      <div class="px-6 py-5 border-b ${theme.panel}">
        <h2 class="text-2xl font-black tracking-tight uppercase flex items-center gap-2"><span class="stepper-extra-tab-icon">${iconShoe()}</span> Choreography / Featured Dances</h2>
      </div>
      <div class="p-6 sm:p-8 space-y-5">
        <div class="rounded-2xl border p-5 ${theme.panel}">
          <p class="text-base sm:text-lg font-bold leading-relaxed">Dances you make with Step by Stepper are collected here automatically on this device. Want one featured as well? Tap the Gmail button and send it in for Featured Dances.</p>
          <div class="mt-4 flex flex-wrap items-center gap-3">
            <a href="${escapeHtml(currentSubmitHref)}" class="inline-flex items-center justify-center rounded-full px-5 py-3 text-xs font-black uppercase tracking-widest ${theme.dark ? 'bg-orange-500 text-neutral-950' : 'bg-orange-500 text-white'}">Submit current dance via Gmail</a>
            <p class="text-sm ${theme.subtle}">The email opens with the dance details already filled in. Just add the featured-dances Gmail address and send it off.</p>
          </div>
        </div>
        ${cards}
      </div>
  ...
  function setActiveExtra(pageName){
    activeExtraPage = pageName === 'whatsnew' ? 'whatsnew' : pageName === 'saveddances' ? 'saveddances' : null;
    ensureHost();
