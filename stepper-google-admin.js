(function(){
  ...
  handleGoogleCredential = async function(response){
    ...
    if (isAdminSession()) await refreshModeratorApplications().catch(() => []);
    renderPages();
  };
  ...
  function decorateAdminPage(){
    ...
    if (!page.querySelector('[data-stepper-moderator-apps="1"]')) {
      const section = document.createElement('div');
      section.setAttribute('data-stepper-moderator-apps', '1');
      section.className = `rounded-3xl border p-5 sm:p-6 ${theme.panel}`;
      const appsHtml = state.moderatorApplications.length ? state.moderatorApplications.map(item => `
        ...
        </article>`).join('') : `<p class="text-sm ${theme.subtle}">No pending moderator applications.</p>`;
      section.innerHTML = `<div class="flex flex-wrap items-center justify-between gap-4"><div><div class="text-lg font-black tracking-tight">Moderator applications</div><p class="mt-1 text-sm ${theme.subtle}">Approve or decline people who want moderator access. Approved moderators get premium perks, but not the Admin tab.</p></div><span class="stepper-google-pill ${theme.orange}">${escapeHtml(String(state.moderatorApplications.length))} pending</span></div><div class="mt-4 grid gap-3">${appsHtml}</div>`;
      const container = page.querySelector('.p-6.sm\:p-8') || page.querySelector('.p-6');
      if (container) container.insertBefore(section, container.children[2] || null);
      section.querySelectorAll('[data-stepper-modapp-id]').forEach(card => {
...
Newest user question: ${question}`;
  ...
  setInterval(() => {
    if (state.session && state.session.credential) {
      refreshSubscription();
      if (isModeratorSession()) refreshModeratorQueue();
      if (isAdminSession()) refreshModeratorApplications();
    }
  }, 12000);
